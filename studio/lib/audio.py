...
def load_input_audio(audio_path, sr=None, **kwargs):
    sound = librosa.load(audio_path, sr=sr, **kwargs)
    return sound
...
def pad_audio(*audios, axis=0):
    max_len = max(len(a) if a is not None else 0 for a in audios)

    if max_len > 0:
        stack = librosa.util.stack(
            [
                librosa.util.pad_center(data=a, size=max_len)
                for a in audios
                if a is not None
            ],
            axis=axis,
